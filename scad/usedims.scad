...
headmountpolepos= concat(headmountpolepos2, [for (z=[[-1, Hhead/2+1], [1, Hhead/2-1]]) [z[0]*edgex(z[1], Wheadbottom)-z[0]*dwall, z[1]]]); // [[Whead/2 - dwall, Hhead/2 - sf], [-Whead/2 + dwall, Hhead/2 + sf]]);
...
framemountpolepos_l=[for (h=[0,Hframe/2-8, Hframe-40, Hframe]) [edgex(h) - dwall, h]];
framemountpolepos_r=[for (p=framemountpolepos_l) [-p[0], p[1]]];
framemountpolepos_b=[for (i=[-1,1]) [i*18, 0]];
