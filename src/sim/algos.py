...
class RunningNormalizer:
    ...
    def update(self, values: np.ndarray):
        ...
        self.var = self.momentum * self.var + (1 - self.momentum) * (
            batch_var + delta**2 * self.count / total_count
        )
...
class REINFORCE:
    ...
    def update(self, tapes: list[Tape]) -> tuple[np.ndarray, float, float, float]:
        ...
        value_loss_scale = RL_CONFIG["value_loss_scale"]
        total_loss = (
            policy_loss + entropy_scale * entropy_loss + value_loss_scale * value_loss
        )
